...
local function writeVirtualFilenameByWinID(winID, boxed_style, winWidth_arg)
  ...
  local filename = fn.fnamemodify(fn.bufname(bnr),':.') .. status
  if filename:sub(0,-string.len(filename)+2) == "oil" then
    filename = fn.fnamemodify(filename:sub(7),':.')
  end
